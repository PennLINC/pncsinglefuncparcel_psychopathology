...
WorkingFolder = [ProjectFolder '/results/SingleParcellation/SingleAtlas_Analysis'];
...
for m = 1:10242
  ...
  for n = 1:17
    % left hemi
    Probability_lh(m, n) = length(find(sbj_AtlasLabel_lh_Matrix(:, m) == n)) / 790;
    Probability_lh(m, n) = Probability_lh(m, n) * log2(Probability_lh(m, n));
    % right hemi
    Probability_rh(m, n) = length(find(sbj_AtlasLabel_rh_Matrix(:, m) == n)) / 790;
