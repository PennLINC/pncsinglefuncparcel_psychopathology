...
def rCCA_APredictB_Bootstrap(Brain_Matrix_train, Behavior_Matrix_train, Covariates_train, Brain_Matrix_test, Behavior_Matrix_test, Covariates_test, Components_Number, BootStrapTimes, ResultantFolder):
    ...
    for i in np.arange(BootStrapTimes):
        ...
        if not os.path.exists(ResultantFolder_TimeI + '/rCCA_Prediction.mat'):
            ...
            system_cmd = 'python3 -c ' + '\'import sys;\
                ...
                from rCCA_CZ_Random_RegressCovariates import rCCA_APredictB_Bootstrap_Sub;\
                import os;\
                import scipy.io as sio;\
                ...
                rCCA_APredictB_Bootstrap_Sub(Brain_Matrix_train_Mat_Path[0], Behavior_Matrix_train_Mat_Path[0], Covariates_train, Brain_Matrix_test_Mat_Path[0], Behavior_Matrix_test_Mat_Path[0], Covariates_test, Components_Number[0][0], ResultantFolder_TimeI[0])\' ';
...
def rCCA_APredictB_Bootstrap_Sub(Brain_Matrix_train_Mat_Path, Behavior_Matrix_train_Mat_Path, Covariates_train, Brain_Matrix_test_Mat_Path, Behavior_Matrix_test_Mat_Path, Covariates_test, Components_Number, ResultantFolder):
    ...
    rCCA_APredictB(Brain_Matrix_train, Behavior_Matrix_train, Covariates_train, Brain_Matrix_test, Behavior_Matrix_test, Covariates_test, Components_Number, ResultantFolder, 0);

def rCCA_APredictB_Permutation(Brain_Matrix_train, Behavior_Matrix_train, Covariates_train, Brain_Matrix_test, Behavior_Matrix_test, Covariates_test, Components_Number, PermutationTimes, ResultantFolder):
    ...
    for i in np.arange(PermutationTimes):
        ...
        if not os.path.exists(ResultantFolder_TimeI + '/rCCA_Prediction.mat'):
            ...
            system_cmd = 'python3 -c ' + '\'import sys;\
                sys.path.append("' + CodesPath + '");\
                from rCCA_CZ_Random_RegressCovariates import rCCA_APredictB_Permutation_Sub;\
                import os;\
                import scipy.io as sio;\
                Configuration = sio.loadmat("' + ResultantFolder_TimeI + '/Configuration.mat");\
                Brain_Matrix_train_Mat_Path = Configuration["Brain_Matrix_train_Mat_Path"];\
                ...
                Components_Number = Configuration["Components_Number"];\
                ResultantFolder_TimeI = Configuration["ResultantFolder_TimeI"];\
                rCCA_APredictB_Permutation_Sub(Brain_Matrix_train_Mat_Path[0], Behavior_Matrix_train_Mat_Path[0], Covariates_train, Brain_Matrix_test_Mat_Path[0], Behavior_Matrix_test_Mat_Path[0], Covariates_test, Components_Number[0][0], ResultantFolder_TimeI[0])\' ';
            system_cmd = system_cmd + ' > "' + ResultantFolder_TimeI + '/Time_' + str(i) + '.log" 2>&1\n'
            script = open(ResultantFolder_TimeI + '/script.sh', 'w');
            script.write(system_cmd);
            script.close();
...
def rCCA_APredictB_Permutation_Sub(Brain_Matrix_train_Mat_Path, Behavior_Matrix_train_Mat_Path, Covariates_train, Brain_Matrix_test_Mat_Path, Behavior_Matrix_test_Mat_Path, Covariates_test, Components_Number, ResultantFolder):
    ...
    data = sio.loadmat(Behavior_Matrix_test_Mat_Path);
    Behavior_Matrix_test = data['Behavior_Matrix_test'];
    rCCA_APredictB(Brain_Matrix_train, Behavior_Matrix_train, Covariates_train, Brain_Matrix_test, Behavior_Matrix_test, Covariates_test, Components_Number, ResultantFolder, 1);

def rCCA_APredictB(Brain_Matrix_train, Behavior_Matrix_train, Covariates_train, Brain_Matrix_test, Behavior_Matrix_test, Covariates_test, Components_Number, ResultantFolder, Permutation_Flag):
    ...
    rcca_model = rcca.CCA(reg = 0.1, numCC = Components_Number);
    rcca_model.train([Brain_Matrix_train, Behavior_Matrix_train]);
    Brain_test_ca = np.dot(Brain_Matrix_test, rcca_model.ws[0]);
    Behavior_test_ca = np.dot(Behavior_Matrix_test, rcca_model.ws[1]);
    ...
    Corr_Training = rcca_model.cancorrs;
    ...
    Brain_Weight = rcca_model.ws[0];
    Behavior_Weight = rcca_model.ws[1]; #'Covariances': Covariances,
    rCCA_result = {'Corr_Training': Corr_Training, \
                  'Corr_Testing': Corr_Testing, \
    ...
    rCCA_FileName = 'rCCA_Prediction.mat'
    ResultantFile = os.path.join(ResultantFolder, rCCA_FileName)
    sio.savemat(ResultantFile, rCCA_result)

def rCCA_Pattern_Bootstrap(Brain_Matrix, Behavior_Matrix, Covariates, Components_Number, BootStrapTimes, ResultantFolder):
    ...
    for i in np.arange(BootStrapTimes):
        ...
        if not os.path.exists(ResultantFolder_TimeI + '/Weight.mat'):
            ...
            system_cmd = 'python3 -c ' + '\'import sys;\
                sys.path.append("' + CodesPath + '");\
                from rCCA_CZ_Random_RegressCovariates import rCCA_Pattern_Bootstrap_Sub;\
                import os;\
                import scipy.io as sio;\
                Configuration = sio.loadmat("' + ResultantFolder_TimeI + '/Configuration.mat");\
                ...
                Components_Number = Configuration["Components_Number"];\
                ResultantFolder_TimeI = Configuration["ResultantFolder_TimeI"];\
                rCCA_Pattern_Bootstrap_Sub(Brain_Matrix_Mat_Path[0], Behavior_Matrix_Mat_Path[0], Covariates, Components_Number[0][0], ResultantFolder_TimeI[0])\' ';
            system_cmd = system_cmd + ' > "' + ResultantFolder_TimeI + '/Time_' + str(i) + '.log" 2>&1\n'
            script = open(ResultantFolder_TimeI + '/script.sh', 'w');
            script.write(system_cmd);
            script.close();
...
def rCCA_Pattern_Bootstrap_Sub(Brain_Matrix_Mat_Path, Behavior_Matrix_Mat_Path, Covariates, Components_Number, ResultantFolder):
    ...
    rCCA_Pattern(Brain_Matrix, Behavior_Matrix, Covariates, Components_Number, ResultantFolder);

def rCCA_Pattern(Brain_Matrix, Behavior_Matrix, Covariates, Components_Number, ResultantFolder):
