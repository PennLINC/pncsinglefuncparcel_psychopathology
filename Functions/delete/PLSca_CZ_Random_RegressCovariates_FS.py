    ...
    for i in np.arange(CVRepeatTimes):
        ...
        if not os.path.exists(ResultantFolder_TimeI + '/Res_NFold.mat'):
            ...
            system_cmd = 'python3 -c ' + '\'import sys;\
                ...
                from PLSca_CZ_Random_RegressCovariates_FS import PLSca_KFold_RandomCV_MultiTimes_Sub;\
                import os;\
                import scipy.io as sio;\
            ...
            Finish_File.append(ResultantFolder_TimeI + '/Fold_1_Score.mat');
...
def PLSca_KFold_RandomCV(Brain_Matrix, Behavior_Matrix, Covariates, Components_Number, Fold_Quantity, ResultantFolder, Permutation_Flag):
    ...
    for j in np.arange(Fold_Quantity):
        ...
        for k in np.arange(Features_Quantity):
            ...
            Coef_Matrix = np.matrix(Coefficients[1:Covariates_Quantity + 1])
            Brain_test[:,[k]] = Brain_test[:,[k]] - np.dot(Covariates_test, np.transpose(Coef_Matrix));
        # Regress covariates from each behavior feature
        for k in np.arange(Behavior_Quantity):
            df['Data'] = Behavior_train[:,k];
            LinModel_Res = sm.ols(formula=Formula, data=df).fit()
            Behavior_train[:,k] = LinModel_Res.resid;
            Coefficients = LinModel_Res.params;
            Behavior_test[:,k] = Behavior_test[:,k] - Coefficients[0];
            Coef_Matrix = np.matrix(Coefficients[1:Covariates_Quantity + 1])
            Behavior_test[:,[k]] = Behavior_test[:,[k]] - np.dot(Covariates_test, np.transpose(Coef_Matrix));

        Training_Quantity = np.shape(Behavior_train)[0]

        # Feature selection
        Median_train = np.median(Brain_train, axis=0);
        Deviation_train = np.abs(Brain_train - np.matlib.repmat(Median_train, Training_Quantity, 1));
        MAD_train = np.median(Deviation_train, axis=0);
        Retain_Quantity = round(Features_Quantity * 0.5);
        Sort_Index = np.argsort(MAD_train);
        FS_Retain_Index = Sort_Index[Features_Quantity - Retain_Quantity + 1:Features_Quantity];
        Brain_train = Brain_train[:, FS_Retain_Index];
        Brain_test = Brain_test[:, FS_Retain_Index];
        ...
        Fold_J_result = {'Index': Fold_J_Index, 'Fold_J_Corr': Fold_J_Corr, \
                         ...
                         'Fold_J_Corr_Training': Fold_J_Corr_Training, 'FS_Retain_Index': FS_Retain_Index}
