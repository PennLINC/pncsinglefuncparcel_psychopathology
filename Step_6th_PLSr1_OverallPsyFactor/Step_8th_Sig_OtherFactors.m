...
PLSr1_Folder = [ProjectFolder '/results/PLSr1/AtlasLoading'];
...
ProjectFolder = '/cbica/projects/GURLAB/projects/pncSingleFuncParcel_psycho';
PLSr1_Folder = [ProjectFolder '/results/PLSr1/AtlasLoading'];
PhobiasFolder = [PLSr1_Folder '/PhobiasFactor_All_RegressCovariates_RandomCV'];
for i = 1:101
  tmp = load([PhobiasFolder '/Time_' num2str(i - 1) '/Res_NFold.mat']);
  Corr_PhobiasFactor_Actual(i) = tmp.Mean_Corr;
  MAE_PhobiasFactor_Actual(i) = tmp.Mean_MAE;
end
PermCell = g_ls([PLSr1_Folder '/PhobiasFactor_All_RegressCovariates_RandomCV_Permutation/*/Res_NFold.mat']);
for i = 1:length(PermCell)
  tmp = load(PermCell{i});
  Corr_PhobiasFactor_Perm(i) = tmp.Mean_Corr;
  MAE_PhobiasFactor_Perm(i) = tmp.Mean_MAE;
end
Corr_PhobiasFactor_P = length(find(Corr_PhobiasFactor_Perm > median(Corr_PhobiasFactor_Actual))) / length(Corr_PhobiasFactor_Perm)
MAE_PhobiasFactor_P = length(find(MAE_PhobiasFactor_Perm < median(MAE_PhobiasFactor_Actual))) / length(MAE_PhobiasFactor_Perm)
save([PLSr1_Folder '/PhobiasFactor_All_RegressCovariates_RandomCV/2Fold_RandomCV_Corr_MAE_Actual_PhobiasFactor.mat'], 'Corr_PhobiasFactor_Actual', 'MAE_PhobiasFactor_Actual', 'Corr_PhobiasFactor_Perm', 'MAE_PhobiasFactor_Perm', 'Corr_PhobiasFactor_P', 'MAE_PhobiasFactor_P')
