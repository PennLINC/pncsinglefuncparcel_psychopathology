...
Prediction_Res = readMat(paste0(PredictionFolder, '/PhobiasFactor_All_RegressCovariates_RandomCV/2Fold_RandomCV_Corr_MAE_Actual_FearFactor.mat'));
Corr_Actual = Prediction_Res$Corr.FearFactor.Actual;

# Find median value
MedianIndex = which(Corr_Actual == median(Corr_Actual));
MedianFolder = paste0(PredictionFolder, '/PhobiasFactor_All_RegressCovariates_RandomCV/Time_', 
...
Fig <- Fig +
       ...
       scale_x_continuous(limits = c(-2.5, 3.5), breaks = c(-2, 0, 2))
Fig

ggsave('/Users/zaixucui/Dropbox/Paper_Writing_Postdoc/pncSingleParcellation_Psychopathology_PLSR/Figures/Figure8_OtherFactors/FearFactorPrediction_CorrACC.tiff', width = 16, height = 15, dpi = 600, units = "cm");
