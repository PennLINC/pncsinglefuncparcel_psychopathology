...
# Actual accuracy
Prediction_Res = readMat(paste0(PredictionFolder, '/PsychosisCorrtraits_All_RegressCovariates_RandomCV/2Fold_RandomCV_Corr_MAE_Actual_Psychosis.mat'));
Corr_Actual = Prediction_Res$Corr.Psychosis.Actual;

# Find median value
MedianIndex = which(Corr_Actual == median(Corr_Actual));
MedianFolder = paste0(PredictionFolder, '/PsychosisCorrtraits_All_RegressCovariates_RandomCV/Time_', 
...
Fig <- Fig +
       ...
       scale_x_continuous(limits = c(-2, 3.1), breaks = c(-2, 0, 2))
Fig
ggsave('/Users/zaixucui/Dropbox/Paper_Writing_Postdoc/pncSingleParcellation_Psychopathology_PLSR/Figures/Figure3_PredictionAccuracy/PsychosisPrediction_CorrACC.tiff', width = 17, height = 15, dpi = 600, units = "cm");
