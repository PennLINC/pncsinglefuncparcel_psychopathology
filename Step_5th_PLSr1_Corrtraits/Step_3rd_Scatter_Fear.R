...
Prediction_Res = readMat(paste0(PredictionFolder, '/FearCorrtraits_All_RegressCovariates_RandomCV/2Fold_RandomCV_Corr_MAE_Actual_Fear.mat'));
Corr_Actual = Prediction_Res$Corr.Fear.Actual;

# Find median value
MedianIndex = which(Corr_Actual == median(Corr_Actual));
MedianFolder = paste0(PredictionFolder, '/FearCorrtraits_All_RegressCovariates_RandomCV/Time_', 
...
Fig <- Fig +
       ...
       scale_x_continuous(limits = c(-2.1, 4.5), breaks = c(-2, 0, 2, 4))
Fig
ggsave('/Users/zaixucui/Dropbox/Paper_Writing_Postdoc/pncSingleParcellation_Psychopathology_PLSR/Figures/Figure3_PredictionAccuracy/FearPrediction_CorrACC.tiff', width = 17, height = 15, dpi = 600, units = "cm");
