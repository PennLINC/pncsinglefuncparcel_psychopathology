...
Prediction_Res = readMat(paste0(PredictionFolder, '/MoodCorrtraits_All_RegressCovariates_RandomCV/2Fold_RandomCV_Corr_MAE_Actual_Mood.mat'));
Corr_Actual = Prediction_Res$Corr.Mood.Actual;

# Find median value
MedianIndex = which(Corr_Actual == median(Corr_Actual));
MedianFolder = paste0(PredictionFolder, '/MoodCorrtraits_All_RegressCovariates_RandomCV/Time_', 
...
Fig <- Fig +
       ...
       scale_x_continuous(limits = c(-2.2, 4.3), breaks = c(-2, 0, 2, 4))
Fig
ggsave('/Users/zaixucui/Dropbox/Paper_Writing_Postdoc/pncSingleParcellation_Psychopathology_PLSR/Figures/Figure3_PredictionAccuracy/MoodPrediction_CorrACC.tiff', width = 17, height = 15, dpi = 600, units = "cm");
