...
do
  ...
  if [ ${BBLID} == \"bblid\" ] || [ "X${SurfPath}" != "X" ]; then
    ...
    qsub -V -e ${logs} -o ${logs} -q all.q -S /bin/bash /cbica/projects/GURLAB/projects/pncSingleFuncParcel_psycho/scripts/Functions/CZ_FSProjectToSurface.sh ${restingDir} ${targDir_resting} ${BBLID} ${ScanID} ${StructuralFolder}
  fi
done < ${SubjectsIDCSV}
...
do
  ...
  if [ ${BBLID} == \"bblid\" ] || [ "X${SurfPath}" != "X" ]; then
    ...
    qsub -V -e ${logs} -o ${logs} -q all.q -S /bin/bash /cbica/projects/GURLAB/projects/pncSingleFuncParcel_psycho/scripts/Functions/CZ_FSProjectToSurface.sh ${nbackDir} ${targDir_nback} ${BBLID} ${ScanID} ${StructuralFolder}
  fi
done < ${SubjectsIDCSV}
...
do
  ...
  if [ ${BBLID} == \"bblid\" ] || [ "X${SurfPath}" != "X" ]; then
    ...
    qsub -V -e ${logs} -o ${logs} -q all.q -S /bin/bash /cbica/projects/GURLAB/projects/pncSingleFuncParcel_psycho/scripts/Functions/CZ_FSProjectToSurface.sh ${idemoDir} ${targDir_idemo} ${BBLID} ${ScanID} ${StructuralFolder}
  fi
done < ${SubjectsIDCSV}
