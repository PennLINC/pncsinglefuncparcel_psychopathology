...
DataFolder='/cbica/projects/GURLAB/projects/pncSingleFuncParcel_psycho/data';
...
do
  ...
  if [ ${BBLID} == \"bblid\" ] || [ "X${SurfPath}" != "X" ]; then
    ...
    qsub -V -e ${logs} -o ${logs} -q all.q -S /bin/bash /cbica/projects/GURLAB/projects/pncSingleFuncParcel_psycho/scripts/AnalysisScripts/Functions/CZ_FSProjectToSurface_SNR.sh ${restingDir} ${ImagePath} ${targDir_resting} ${BBLID} ${ScanID} ${StructuralFolder}
  fi
done < ${SubjectsIDCSV}
...
do
  ...
  if [ ${BBLID} == \"bblid\" ] || [ "X${SurfPath}" != "X" ]; then
    ...
    qsub -V -e ${logs} -o ${logs} -q all.q -S /bin/bash /cbica/projects/GURLAB/projects/pncSingleFuncParcel_psycho/scripts/AnalysisScripts/Functions/CZ_FSProjectToSurface_SNR.sh ${nbackDir} ${ImagePath} ${targDir_nback} ${BBLID} ${ScanID} ${StructuralFolder}
  fi
done < ${SubjectsIDCSV}
...
do
  ...
  if [ ${BBLID} == \"bblid\" ] || [ "X${SurfPath}" != "X" ]; then
    ...
    qsub -V -e ${logs} -o ${logs} -q all.q -S /bin/bash /cbica/projects/GURLAB/projects/pncSingleFuncParcel_psycho/scripts/AnalysisScripts/Functions/CZ_FSProjectToSurface_SNR.sh ${idemoDir} ${ImagePath} ${targDir_idemo} ${BBLID} ${ScanID} ${StructuralFolder}
  fi
done < ${SubjectsIDCSV}
