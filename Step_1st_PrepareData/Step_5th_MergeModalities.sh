...
do
  ...
  if [ ${BBLID} == \"bblid\" ] || [ "X${ResultantFolder_I_Exist}" != "X" ]; then
    ...
    qsub -V -e ${logs} -o ${logs} -q all.q -S /bin/bash /cbica/projects/GURLAB/projects/pncSingleFuncParcel_psycho/scripts/Functions/CZ_Merge3Modality_FS.sh ${RSFolder} ${NBackFolder} ${EmotionFolder} ${BBLID} ${ResultantFolder_I}
  fi
done < ${SubjectsIDCSV}
